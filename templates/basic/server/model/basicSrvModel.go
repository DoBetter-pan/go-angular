...
var sqls map[string] string = map[string] string {
    "query":"##query##",
    "queryone":"##queryone##",
    "insert":"##insert##",
    "update":"##update##",
...
func (model *BasicSrvModel) FindAll() (string, error) {
    ...
    rows, err := dbconnection.DB.Query(sqls["query"])
    if err != nil {
        return "", err
    }
    defer rows.Close()
...
func (model *BasicSrvModel) Find(id int64) (string, error) {
    var basic Basic
    dbconnection := dbwrapper.GetDatabaseConnection()
    err := dbconnection.DB.QueryRow(sqls["queryone"], id).Scan(##scanitemlist##)
    if err != nil {
        return "", err
    }
...
func (model *BasicSrvModel) Insert(str string) (string, error) {
    ...
    res, err := tx.Exec(sqls["insert"], ##insertitemlist##)
    if err != nil {
        tx.Rollback()
        return "", err
    }
